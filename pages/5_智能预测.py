...
def _forecast_revenue(daily_df: pd.DataFrame, forecast_days: int = 14, method: str = "随机森林回归"):
    """
    时间序列预测 — 支持三种方法
    ...
    if method == "随机森林回归":
        ...
        method_name = "随机森林回归(RandomForest)"
    elif method == "线性回归":
        model = LinearRegression()
        method_name = "线性回归(LinearRegression)"
    else:  # 移动平均
        recent = df.tail(7)
        avg_val = recent[rev_col].mean()
        trend = (recent[rev_col].iloc[-1] - recent[rev_col].iloc[0]) / max(len(recent) - 1, 1)
        result = pd.DataFrame({
            "date": [(df["date"].max() + timedelta(days=i+1)).strftime("%Y-%m-%d") for i in range(forecast_days)],
            "predicted": [round(max(0, avg_val + trend * (i+1)), 2) for i in range(forecast_days)],
            "lower_bound": [round(max(0, (avg_val + trend * (i+1)) * 0.85), 2) for i in range(forecast_days)],
            "upper_bound": [round(max(0, (avg_val + trend * (i+1)) * 1.15), 2) for i in range(forecast_days)],
        })
        return result, {"mape": None, "method": "移动平均(MovingAverage)", "forecast_days": forecast_days}
